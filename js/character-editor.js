...
const name=(id,detail)=>chineseName(id,detail?.name||id.split('/').pop());
export function createCharacterEditor(host,onApply){
  ...
  function render(){
    ...
    if(!catalog){html+='<p>游戏数据加载后，可编辑装备、技能、触发条件与增益。</p>';}
      ...
      const buffs=Object.entries(BUFF_GROUPS).map(([group,label])=>section(`buff-${group}`,`${label} Buff（${draft.buffs[group].length}）`,draft.buffs[group].map((b,i)=>{
      ...
      html+=section('buffs','各来源 Buff',`<p class="editor-note">比例栏填百分数，例如 5 表示 5%；固定加成按该属性单位填写，暴击率、施法速度等小数属性中 0.01 表示 1 个百分点。这些来源按常驻加成参与模拟；临时增益由技能触发。相同数值的重复增益沿用引擎去重规则。</p>${buffs}`);
      ...
      html+=section('achievements','成就状态',`<p class="editor-note">勾选为已完成；属性增益由战斗引擎根据成就计算。</p><div class="character-edit-grid">${Object.entries(catalog.achievementDetailMap).map(([k,v])=>`<label class="editor-check"><input type="checkbox" data-edit-path="${esc(JSON.stringify(['achievements',k]))}" ${draft.achievements[k]?'checked':''}>${esc(name(k,v))}</label>`).join('')}</div>`);
