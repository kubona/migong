...
当前版本：v0.43。访问：https://kubona.github.io/migong/

v0.43 完整合法配装及主动顺序参与竞争。模型从正常战斗结果边模拟边学习，只安排测试先后，不截断候选。每套先测试当前最高已确认等级，通过后增加5级，失败后二分。上探点达到本轮额度仍不确定时，继续细化已通过等级到临界点之间的整数等级，不将临界点视为失败上界。
...
首次默认100场、每批追加300场，每个配装等级本轮最多5000场。仍未确定时保留候选，可恢复任务追加新样本，不将其判为失败或宣布最优。当前最高等级所有已认证方案进行等场数独立排名复核，默认每套1000场，可调整；用新样本比较胜率与胜场平均耗时。复核与搜索证据分开，不进入学习或搜索认证。尚未确定最高等级时，页面和导出明确标注未完成。完整候选分页保存在本机，内存工作窗口不是搜索范围上限。旧版断点不能跨计算版本恢复；兼容学习档案可引导排序，不直接充当新任务认证证据。
